...
session_start();
$id=$_SESSION['id'];
$res= mysqli_query($conn, "SELECT aid from administrator");
$roww= mysqli_fetch_assoc($res);
$res1= mysqli_query($conn, "SELECT department from administrator");
$roww1= mysqli_fetch_assoc($res1);
$res2= mysqli_query($conn, "SELECT designation from administrator");
$roww2= mysqli_fetch_assoc($res2);
$res3= mysqli_query($conn, "SELECT name from administrator");
$roww3= mysqli_fetch_assoc($res3);
$result1=mysqli_query($conn,"SELECT bid FROM administrator");
...
<div class="container">
    <?php 
    $ress = mysqli_query($conn, "SELECT * from administrator");
    while($row= mysqli_fetch_assoc($ress)) { ?>
    <table class="table">
      <tr class="row m-0">
        <th class="col-sm-2">Administrator ID</th>
        <td class="col-sm-10"><?php echo $row["aid"];?></td>
      </tr>
      <tr class="row m-0">
      ...
      <tr class="row m-0">
        ...
        <td class="col-sm-10"><?php echo $row["department"];?></td>
      </tr>
      <tr class="row m-0">
    ...
    <?php }?>
    <center>
    <td class="col-sm-1"><input id="edit" type="button" class="btn" value="Change Password"/></td>
    </center>
</div>
