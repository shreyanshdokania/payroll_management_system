...
session_start();
$id=$_SESSION['id'];
$res= mysqli_query($conn, "SELECT * from employee where eid='$id';");
$roww= mysqli_fetch_assoc($res);
$aidd=$roww['aid'];
$res1= mysqli_query($conn, "SELECT ename from employee where eid='$id'");
$roww1= mysqli_fetch_assoc($res1);
$res2= mysqli_query($conn, "SELECT dob from employee where eid='$id'");
$roww2= mysqli_fetch_assoc($res2);
$res3= mysqli_query($conn, "SELECT phone_no from employee where eid='$id'");
$roww3= mysqli_fetch_assoc($res3);
$result=mysqli_query($conn,"SELECT aid FROM administrator");
$row=mysqli_fetch_assoc($result);
$aid=$row['aid'];
$result1=mysqli_query($conn,"SELECT * FROM administrator WHERE aid='$aidd';");
...
$resultb=mysqli_query($conn,"SELECT basic FROM salary");
$rowb=mysqli_fetch_assoc($resultb);
$basic=$rowb["basic"];
$result5=mysqli_query($conn,"SELECT * FROM salary WHERE aid=$aidd;");
$row5=mysqli_fetch_assoc($result5);
$res1= mysqli_query($conn, "SELECT designation from administrator where aid='$aidd'");
$rowd= mysqli_fetch_assoc($res1);
$res2= mysqli_query($conn, "SELECT basic from employee where eid-'$id'");
$rowbb= mysqli_fetch_assoc($res2);
...
<div class="container">
  <table class="table">
      <tr class="row m-0">
        ...
        <td class="col-sm-10"><?php echo $roww["eid"];?></td>
      </tr>
      <tr class="row m-0">
        <th class="col-sm-2">Employee Name</th>
        <td class="col-sm-10"><?php echo $roww1["ename"];?></td>
      </tr>
      <tr class="row m-0">
        <th class="col-sm-2">Date Of Birth</th>
        <td class="col-sm-10"><?php echo $roww2["dob"];?></td>
      </tr>
      <tr class="row m-0">
        <th class="col-sm-2">Phone Number</th>
        <td class="col-sm-10"><?php echo $roww3["phone_no"];?></td>
      </tr>
      <tr class="row m-0">
      ...
      <tr class="row m-0">
        ...
        <td class="col-sm-10"><?php echo $rowd["designation"];?></td>
      </tr>
      <tr class="row m-0">
        <th class="col-sm-2">Basic</th>
        <td class="col-sm-10"><?php echo $row5["basic"];?></td>
      </tr>
      <tr class="row m-0">
